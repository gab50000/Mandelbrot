...
class imageSection:
    ...
    def resize(self, x, y):
        ...
        self.rect.height = int(
            float(self.rect.width)
            * self.surface.get_size()[1]
            / self.surface.get_size()[0]
        )
        if (y < self.anchor[1] and x > self.anchor[0]) or (
            y > self.anchor[1] and x < self.anchor[0]
        ):
            self.rect.height *= -1
...
def colorfunct(convergecolor, divergecolor, iters, maxiter):
    R = int(
        divergecolor[0] + float(iters) / maxiter * (convergecolor[0] - divergecolor[0])
    )
    G = int(
        divergecolor[1] + float(iters) / maxiter * (convergecolor[1] - divergecolor[1])
    )
    B = int(
        divergecolor[2] + float(iters) / maxiter * (convergecolor[2] - divergecolor[2])
    )
    ...
    cvalmat = np.array(
        [
            [
                complex(
                    clipping[0].real
                    + j * (clipping[1].real - clipping[0].real) / size[0],
                    clipping[0].imag
                    + i * (clipping[1].imag - clipping[0].imag) / size[1],
                )
...
def reclip(clipping, rect, size):
    clip_real = (clipping[1].real - clipping[0].real)
    left = clip_real * float(rect.left) / size[0] + clipping[0].real
    right = clip_real * float(rect.right) / size[0] + clipping[0].real

    clip_imag = (clipping[1].imag - clipping[0].imag)
    up = clip_imag * float(rect.bottom) / size[1] + clipping[0].imag
    down = clip_imag * float(rect.top) / size[1] + clipping[0].imag

    newclip = (
        complex(min(left, right), max(up, down)),
        complex(max(left, right), min(up, down)),
    )
    return newclip
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "-i", "--maxiter", type=int, default=300, help="Maximum number of iterations"
    )
    parser.add_argument(
        "-b",
        "--boundaries",
        type=complex,
        default=(-2.5 + 1.5j, 1.5 - 1.5j),
        nargs=2,
        help="Image section at start (imaginary number at upper left and at bottom right boundary",
    )
    ...
    colormap = [
        imagcolor(float(itersteps) / args.maxiter * 180, 180)
        for itersteps in range(args.maxiter + 1)
    ]
            ...
            for event in pygame.event.get():
                if event.type == pl.MOUSEBUTTONUP:
                    if not ausschnitt:
                        ...
                        ausschnitt = None
                        calc_clipping(
                            mandelSurface, complmat, limitmat, clipping, maxiter
                        )
                        draw_iter(mandelSurface, limitmat, maxiter, *size, colormap)
